...
export default function AppointmentForm(){
 ...
 return <form className="booking-form" onSubmit={submit}>
  <div className="booking-section"><div className="booking-heading"><span>1</span><div><h2>Choose doctor and service / Doctor aur service chuniye</h2><p>Aap kisi bhi available doctor ko bhi chun sakte hain.</p></div></div><div className="field-grid"><label>Doctor<select name="doctor" required defaultValue=""><option value="" disabled>Doctor chuniye</option><option>Doctor 1 — details pending</option><option>Doctor 2 — details pending</option><option>Doctor 3 — details pending</option><option>Koi bhi available doctor</option></select></label><label>Baby Care Service<select name="service" required defaultValue=""><option value="" disabled>Service chuniye</option><option>Baby Health Check</option><option>Vaccination Guidance</option><option>Common Illness Care</option><option>Growth & Nutrition</option><option>Samajh nahi aa raha</option></select></label></div></div>
  <div className="booking-section"><div className="booking-heading"><span>2</span><div><h2>Choose date / Din chuniye</h2><p>Appointment clinic confirmation ke baad pakka hoga.</p></div></div><label className="date-field">Preferred Date<input name="date" type="date" min={today} value={date} onChange={e=>setDate(e.target.value)} required/></label></div>
  <div className="booking-section"><div className="booking-heading"><span>3</span><div><h2>Choose time / Samay chuniye</h2><p>Blue slot aapka selected time hai.</p></div></div><div className="slot-group"><h3>☀️ Subah</h3><div className="slot-grid">{slots(morning)}</div></div><div className="slot-group"><h3>🌇 Shaam</h3><div className="slot-grid">{slots(evening)}</div></div><input type="hidden" name="time" value={time}/></div>
  <div className="booking-section"><div className="booking-heading"><span>4</span><div><h2>Enter contact details / Contact details bhariye</h2><p>Clinic team isi number par confirmation degi.</p></div></div><div className="field-grid"><label>Parent / Guardian Name<input name="name" autoComplete="name" required minLength={2} placeholder="Apna poora naam"/></label><label>Mobile Number<input name="phone" type="tel" inputMode="tel" autoComplete="tel" required pattern="[0-9 +()-]{8,18}" placeholder="+91 00000 00000"/></label><label>Email <small>(optional)</small><input name="email" type="email" autoComplete="email" placeholder="name@example.com"/></label><label>Baby ki age <small>(optional)</small><input name="age" placeholder="Jaise: 8 months"/></label></div><label>Bachche ke baare mein chhoti baat <small>(private medical details na likhein)</small><textarea name="message" rows={3} placeholder="Jaise: general check-up"/></label></div>
  ...
  <div className="booking-submit"><div><b>Selected:</b><span>{date||"Choose date / Din chuniye"} • {time||"Choose time / Samay chuniye"}</span></div><button className="button big-button" type="submit" disabled={sending}>{sending?"Submit ho raha hai…":"Appointment Request Submit Karein →"}</button></div>
